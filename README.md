Autor: David Alves da Silva.
Professor de Sistemas Operacionais (quem propôs os exercícios): Prof Dr. Leandro Colevati dos Santos.

Exercício 1: Cálculo da área de um quadrado a partir do seu lado.
Exercício 2: Aplicação de reajuste de 15% sobre um salário.
Exercício 3: Cálculo da área de um triângulo com base e altura.
Exercício 4: Conversão de temperatura de Celsius para Fahrenheit.
Exercício 5: Cálculo das raízes reais de uma equação do 2º grau.
Exercício 6: Troca direta de valores entre duas variáveis.
Exercício 7: Cálculo do volume de um paralelepípedo.
Exercício 8: Cálculo de rendimento da poupança após 1 mês (1,3% a.m.).
Exercício 9: Soma dos quadrados de dois números inteiros.
Exercício 10: Cálculo da diferença entre dois números reais.
Exercício 11: Cálculo do comprimento de uma circunferência pelo raio.
Exercício 12: Cálculo da idade atual e projeção para daqui a 17 anos.
Exercício 13: Duração em dias de um estoque de alimento (consumo de 50g/dia).
Exercício 14: Determinação do 3º ângulo interno de um triângulo.
Exercício 15: Cálculo da hipotenusa em um triângulo retângulo.
Exercício 16: Cálculo do salário líquido com desconto e acréscimo por dependente.
Exercício 17: Cálculo do consumo de combustível com base em tempo, velocidade e autonomia.
